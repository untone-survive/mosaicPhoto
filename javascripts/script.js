...
(function ($) {
  ...
  var mosaicPhoto = function (root, settings) {
    ...
    function _newColsRows() {
      ...
      var newCols = Math.floor(container.width() / imgWidth), newRows = Math.floor(container.height() / imgHeight);
    ...
    function _populate() {
      ...
      // get random image positions
        divs = [], totalDivs = settings.rows * settings.cols;
      ...
      var position = 0, div = divs.shift(), img = imgs.shift(), imgString = '';
      ...
      for (i = 0; i < settings.rows; i++) {
        for (var j = 0; j < settings.cols; j++) {
          ...
          if (divs.length) {
            ...
            if (position === div) {
              var isObject = typeof img === 'object', src = isObject ? img[0] : img, // if link is provided, generate it
      ...
      // clear container
      _clearContainer();
      $(imgString).appendTo(container).fadeIn(settings.fadeTime);// fade in all added photos.fadeIn(settings.fadeTime);
    ...
    function _setupListeners() {
      ...
      if (listeners.length) {
        $document.delegate(settings.listeners, settings.listenersEvents + namespace, function (e) {
    ...
    function _setMaxImages() {
      maxImages = Math.min(Math.floor(settings.rows * settings.cols * settings.ratio), settings.maxImages);
    }
  ...
  $.fn.mosaicPhoto = function (mp_settings) {
    ...
    if (data) {
      ...
      if (mp_settings.preload) {
        $.each(mp_settings.images, function (idx, value) {
          $.preloadMosaicPhotoImages(value);
        });
      }
      return this;
    }
  ...
  $.preloadMosaicPhotoImages = function () {
    var images = arguments[0],
      n = images.length;

    for (var i = 0; i < n; i++) {
      var src = typeof images[i] === 'object' ? images[i][0] : images[i];
